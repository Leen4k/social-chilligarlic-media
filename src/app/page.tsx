  ...
  return (
    <div className="h-screen overflow-scroll col-span-10 lg:col-span-6 border-r-2 border-slate-100">
      ...
      <div className="flex flex-col h-screen overflow-scroll">
        {data?.map((post)=>(
          <Post heart={post.hearts} photos={post.photos} key={post.id} id={post.id} comments={post.comments} postTitle={post.title} name={post.user.name} avatar={post.user.image} />
        ))} 
      </div>
    </div>
  )
